...
// Coded by kei
  ...
  const entry = await client.guilds.cache.get(cfg.Server.GuildID).fetchAuditLogs({type: 'CHANNEL_UPDATE'}).then(audit => audit.entries.first())
  ...
  closeAllPermsRoles()
  client.logSend(`${entry.executor} üyesi **kanal güncelledi** ve kanalı eski haline getirip, kanalı güncelleyen kişiyi banladım.\n\n\`⦁\` Kanal: <#${oldChannel.id}> (\`${oldChannel.name}\` - \`${oldChannel.id}\`)\n\`⦁\` Yetkili: ${entry.executor} (\`${entry.executor.tag.replace("`", "")}\` - \`${entry.executor.id}\`)\n\n\`-\` Tarih: \`${Tarih}\``)
// Coded by kei
